...
class PolynomialRegression():
    ...
    def fit(self, features, targets):
        ...
        """
        X = np.random.uniform(-1, 1, size=(features.shape[0], self.degree+1))
        for i in range(features.shape[0]):
            for j in range(self.degree+1):
                X[i, j] = features[i] ** j
        
        w = (np.linalg.inv(((X.T).dot(X))).dot(X.T)).dot(targets)
        self.w = w
    ...
    def predict(self, features):
        ...
        #Y = np.random.uniform(-1, 1, size=(features.shape[0]))
        predictions = np.random.uniform(-1, 1, size=(features.shape[0]))
        
        for i in range(features.shape[0]):
            predictions[i] = 0.0
            predictions[i] += self.w[0]
            for j in range(1, self.degree+1):
                predictions[i] += self.w[j] * (features[i] ** j)
