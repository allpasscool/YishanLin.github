...
class GMM():
    ...
    def _e_step(self, features):
        ...
        for i in range(self.n_clusters):
            tmp = self._posterior(features, i).reshape(-1, 1)

            if i == 0:
                post_prob = tmp
            else:
                post_prob = np.c_[post_prob, tmp]
    ...
    def _m_step(self, features, assignments):
        ...
        mixing_weight = self.mixing_weights

        for i in range(self.n_clusters):
            tmp_sum = np.sum(assignments[:, i])
            
            mean[i] = features.T.dot(assignments[:, i]) / tmp_sum
            mixing_weight[i] = tmp_sum / features.shape[0]
            tmp_covariance = ((features - mean[i]) ** 2).T.dot(assignments[:, i]) / tmp_sum

            if self.covariance_type == 'spherical':
                covariance[i] = np.mean(tmp_covariance)
            elif self.covariance_type == 'diagonal':
                covariance[i] = tmp_covariance
        
        return mean, covariance, mixing_weight
