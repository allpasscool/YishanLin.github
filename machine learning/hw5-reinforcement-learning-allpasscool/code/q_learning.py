...
class QLearning:
    ...
    def fit(self, env, steps=1000):
        ...
        for i in range(steps):
          ...
          # env.render()
          r = np.random.rand()
          epilson_now = self._get_epsilon(i/steps)

          if r < (1 - epilson_now):
            action = np.argmax(Q[state, :])
            if action == 0:
              index = np.where(Q[state, :] == Q[state, action])[0]
              action = index[np.random.randint(len(index))]
          else:
          ...
          Q[state, action] += (1 / N[state, action]) * (reward + self.discount * np.max(Q[observation, :]) - Q[state, action])
