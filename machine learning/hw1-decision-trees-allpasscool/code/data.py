...
def load_data(data_path):
    ...
    #count rows
    rows_count = sum(1 for row in csv.reader(open(data_path)))
    rows_count -= 1     #ignore first row
    ...
    features = np.random.uniform(0, 10, size=(rows_count, columns_count-1))
    targets = np.random.randint(0, 20, size=(rows_count))
    attribute_names = []
    ...
    with open(data_path) as csvfile:
        ...
        for i in range(0, columns_count-1):
            attribute_names.append(first_row[i])
        ...
        for row in readCSV:
            ...
            for i in range(0, columns_count-1):
                tmp.append(row[i])
            features[counter_now] = tmp
            targets[counter_now] = row[columns_count-1]
            counter_now += 1
