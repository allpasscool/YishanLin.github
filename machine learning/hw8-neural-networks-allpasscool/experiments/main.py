...
test_features = test_features.reshape(-1,3, 64, 64)
test_features = torch.from_numpy(test_features)
test_features = test_features.reshape(test_features.size()[0],3,64,64)
...
# val_features = val_features.reshape(val_features.size()[0],3,64,64)
...
for i in data_size:
    ...
    tmp = tmp.reshape(nexample, 3, 64, 64)

    train_features1, train_targets1 = tmp, dog.trainY
    train_features1 = torch.from_numpy(train_features1).float()
    train_features1 = train_features1.reshape(train_features1.size()[0],3,64,64)
    ...
    for epoch in range(0, 100):
        ...
        print("vali_epoch: ", vl/total)
        print("total of vali data:", total)
        val_losses.append(vl/total)
...
# Save the figure
fig.savefig('questiondogcnn_losses.png', bbox_inches='tight')
...
# Save the figure
fig.savefig('questiondogcnn_avl_accur.png', bbox_inches='tight')
